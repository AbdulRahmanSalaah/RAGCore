...
class AssetModel(BaseDataModel):
    def __init__(self, db_client: object):
        ...
        self.asset_collection = db_client[DataBaseEnum.COLLECTION_ASSET_NAME.value]
    ...
    async def init_collection(self):
        ...
        if DataBaseEnum.COLLECTION_ASSET_NAME.value not in all_collections:
            self.asset_collection =  self.db_client[DataBaseEnum.COLLECTION_ASSET_NAME.value]
    ...
    async def get_all_project_assets(self, asset_project_id: str, asset_type: str):
        records  = self.asset_collection.find({
            "asset_project_id": ObjectId(asset_project_id) if isinstance(asset_project_id, str) else asset_project_id,
            "asset_type": asset_type
        })

        # convert it to pydantic model list
        assets = []
