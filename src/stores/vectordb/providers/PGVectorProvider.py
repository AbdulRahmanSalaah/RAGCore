...
class PGVectorProvider(VectorDBInterface):
        ...
        async def is_collection_existed(self, collection_name: str) -> bool:
            ...
            async with self.db_client() as session:
                async with session.begin():
                    # SELECT 1 returns a single scalar; safe to use scalar_one_or_none()
                    list_tbl = sql_text(
        ...
        async def list_all_collections(self) -> List:
            ...
            async with self.db_client() as session:
                async with session.begin():
                    # Collection tables are named 'collection_{size}_{project_id}'
                    list_tbl = sql_text(
                        r"SELECT tablename FROM pg_tables WHERE tablename LIKE 'collection\_%'"
                    )
                    results = await session.execute(list_tbl)
                    ...
                    async with session.begin():
                        create_sql = sql_text(
                            f'CREATE TABLE "{collection_name}" ('
                                ...
                                f'FOREIGN KEY ({PgVectorTableSchemeEnums.CHUNK_ID.value}) REFERENCES chunks(id) ON DELETE CASCADE'
            ...
            async with self.db_client() as session:
                async with session.begin():
                    ...
                    if records_count < self.index_threshold:
                        self.logger.info(
                            f"Skipping index creation for '{collection_name}': "
                            f"{records_count} records < threshold {self.index_threshold}"
                        )
                        return False
                ...
                async with session.begin():
                    for i in range(0, len(texts), batch_size):
                        ...
                        for _text, _vector, _metadata, _record_id in zip(batch_texts, batch_vectors, batch_metadata, batch_record_ids):
                            # jsonb column requires a JSON string, not a raw Python dict
                            metadata_json = json.dumps(_metadata, ensure_ascii=False) if _metadata is not None else "{}"
                            values.append({
                                'text': _text,
                                # pgvector expects a vector literal string like "[0.1,0.4,...]", not a Python list
